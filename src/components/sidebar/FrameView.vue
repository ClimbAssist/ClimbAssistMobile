    ...
    <div v-for="(wall, wi) in crag.walls" :key="wi">
      <div v-for="(route, index) in wall.routes" :key="index">
        <v-list-item v-if="index == 0" >
          <v-list-item-content>
            <v-list-item-title class="green--text text--darken-1">{{ crag.walls[wi].name }}</v-list-item-title>
        ...
        >
          <v-list-item-content>
            ...
            <v-list-item-subtitle class="grey--text text--lighten-2"
              >{{ route.style }} {{ route.grade }}{{ route.gradeModifier }}
...
export default {
  ...
  methods: {
    updateActiveRoute(w, r) {
      ...
      this.$store.commit("updateActiveRoute", activeRoute);
    },
    removeActiveRoute() {
