...
export default {
  ...
  methods: {
    ...
    //           let decalDiffuse = textureLoader.load(require('@/assets/decal/decal-diffuse.png'))
    ...
    checkAnnotationOpacity() {
      ...
      for (let key in this.group.children) {
        ...
        let annotationRaycaster = new THREE.Raycaster(raycastPos, raycastDir);
        let annotationIntersects = annotationRaycaster.intersectObject(
          this.mesh
        );

        this.group.children[key].material.opacity =
          annotationIntersects.length > 0 ? 0.5 : 1;
      }
    },
    resetCam() {
      this.controls.reset();
