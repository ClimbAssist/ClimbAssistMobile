  ...
  computed: {
    ...
    subAreaFills() {
      ...
      for (let subkey in this.area.area.subAreas) {
        if (
          this.area.area.subAreas[subkey].crags.length > 1
        ) {
          ...
          for (let ckey in this.area.area.subAreas[subkey].crags) {
            if (
              ...
              minLatitude = this.area.area.subAreas[subkey].crags[ckey]
                .location.latitude;
            }
            if (
              this.area.area.subAreas[subkey].crags[ckey].location.latitude >
              ...
              maxLatitude = this.area.area.subAreas[subkey].crags[ckey]
                .location.latitude;
            }
            if (
          ...
          minLatitude -= offset;
          maxLatitude += offset;
          let titleLoc = [minLongitude + (maxLongitude - minLongitude)/2, maxLatitude]
          let title = {
            type: "Feature",
            geometry: {
              type: "Point",
          ...
          let fill = {
            ...
            geometry: {
              type: "Polygon",
              coordinates: [[[minLongitude, minLatitude],[minLongitude, maxLatitude], [maxLongitude, maxLatitude], [maxLongitude, minLatitude], [minLongitude, minLatitude]]]
            },
          }
  ...
  methods: {
    ...
    createMap() {
      ...
      this.map = new mapboxgl.Map({
        ...
        center: [this.area.area.location.longitude, this.area.area.location.latitude],
        zoom: 13,
        minZoom: 11
      });
