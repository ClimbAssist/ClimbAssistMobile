  ...
  methods: {
    getData() {
      ...
      for (let i in this.$store.state.filter.loadedCrags) {
        if (this.$store.state.filter.loadedCrags[i].cragId === this.$route.params.frame) {
          this.$store.commit("updateCrag", this.$store.state.filter.loadedCrags[i]);
          return;
        }
      }
      //check for locally saved file
      window.requestFileSystem(window.PERSISTENT,  10 * 1024 * 1024, (fs) => {
        fs.root.getFile(this.$route.params.frame + ".json", {create: false}, (fileEntry) => {
          fileEntry.file( (file) => {
    ...
    fetchData() {
      ...
      cordova.plugin.http.get("https://www.climbassist.com/v1/crags/" + this.$route.params.frame + "?depth=4", {}, {}, 
      (response) => {
        //success
    ...
    if (this.$route.params.countryKey) {
      crag.area = {
        name: this.$store.state.filter.countries[this.$route.params.countryKey].regions[this.$route.params.regionKey].areas[this.$route.params.areaKey].name,
        areaId: this.$store.state.filter.countries[this.$route.params.countryKey].regions[this.$route.params.regionKey].areas[this.$route.params.areaKey].areaId
      };
      this.$store.commit("updateCrag", crag);
      this.$store.commit("updateLoadedCrags", crag);
    } else {
      cordova.plugin.http.get("https://www.climbassist.com/v1/sub-areas/" + crag.subAreaId, {}, {}, 
      (response) => {
        //success
        let subArea = JSON.parse(response.data).data
        cordova.plugin.http.get("https://www.climbassist.com/v1/areas/" + subArea.areaId, {}, {},
        (response) => {
          //success
